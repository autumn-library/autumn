...
// Запускает рогатки приложения.
//
// Метод не называется ЗапуститьПриложение: это имя занято глобальным методом движка,
// и нативный компилятор считает одноимённый метод модуля повторным определением.
//
