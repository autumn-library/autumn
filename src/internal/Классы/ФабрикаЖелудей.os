...
Функция СоздатьОпределениеЖелудя(
	...
	Если Спецификация = СостоянияПриложения.Инициализация() Тогда
		Для Каждого ПрилепляемаяЧастица Из ПрилепляемыеЧастицы Цикл
			ПроверитьДопустимостьЧастицыДляИнициализации(ИмяЖелудя, ПрилепляемаяЧастица);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПроверитьДопустимостьЧастицыДляИнициализации(ИмяЖелудя, ДанныеОПрилепляемойЧастице)

	ТипЧастицы = ДанныеОПрилепляемойЧастице.ТипЧастицы();
	ИмяЧастицы = ДанныеОПрилепляемойЧастице.ИмяЧастицы();

	Если ТипЧастицы = ТипыПрилепляемыхЧастиц.Деталька() Тогда
		Возврат;
	КонецЕсли;

	Если ВРег(ИмяЧастицы) = ВРег("Поделка") Тогда
		Возврат;
	КонецЕсли;

	Если ТипЧастицы = ТипыПрилепляемыхЧастиц.Желудь() Тогда
		ОпределениеЗависимости = ПолучитьОпределениеЖелудя(ИмяЧастицы);
		Если ОпределениеЗависимости = Неопределено Тогда
			ВызватьИсключение СтрШаблон(
				"К желудю времени инициализации %1 можно прилеплять только детальки и желуди времени инициализации. Определение желудя %2 не найдено.",
				ИмяЖелудя,
				ИмяЧастицы
			);
		КонецЕсли;

		Если ОпределениеЗависимости.Спецификация() = СостоянияПриложения.Инициализация() Тогда
			Возврат;
		КонецЕсли;

		ВызватьИсключение СтрШаблон(
			"К желудю времени инициализации %1 можно прилеплять только детальки и желуди времени инициализации. Желудь %2 не имеет спецификации ""Инициализация"".",
			ИмяЖелудя,
			ИмяЧастицы
		);
	КонецЕсли;
	...
	ВызватьИсключение СтрШаблон(
		"К желудю времени инициализации %1 можно прилеплять только детальки и желуди времени инициализации.",
		ИмяЖелудя
	);
