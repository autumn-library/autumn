...
Функция ДобавитьДуб(ТипДуба) Экспорт
	...
	ОпределениеДуба = ДобавитьЖителяЛеса(ТипДуба, "", "Дуб");
	МетодыЗавязи = ОпределениеДуба.НайтиМетодыСАннотациями("Завязь");
	...
	Для Каждого МетодЗавязи Из МетодыЗавязи Цикл
		...
		Если Спецификация = СостоянияПриложения.Инициализация() 
			И НЕ ОпределениеДуба.Спецификация() = СостоянияПриложения.Инициализация() Тогда
	...
	Возврат ОпределениеДуба;

КонецФункции

Функция ПостИнициализацияДуба(ТипДуба) Экспорт

	ОпределениеДуба = ПолучитьОпределениеЖелудя(Строка(ТипДуба));

	МетодыЗавязи = ОпределениеДуба.НайтиМетодыСАннотациями("ВеткаСЗавязями");

	Для Каждого МетодЗавязи Из МетодыЗавязи Цикл
		Аннотации = МетодЗавязи.Аннотации;

		Аннотация = РаботаСАннотациями.НайтиАннотацию(Аннотации, "ВеткаСЗавязями");
		ПараметрПоиска = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(Аннотация, "ПараметрПоиска");
		ПрефиксИмениЖелудя = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(Аннотация, "ПрефиксИмениЖелудя");

		ОпределенияЖелуедй = ПолучитьСписокОпределенийЖелудей(ПараметрПоиска);

		Для каждого Определение Из ОпределенияЖелуедй Цикл
			
			ИмяЖелудяЗавязи = ПрефиксИмениЖелудя + Определение.Имя();
			ТипЖелудяЗавязи = ПрочитатьТипЖелудяПИ(ИмяЖелудяЗавязи, Аннотации);
			ХарактерЖелудя = ПрочитатьХарактерЖелудя(Аннотации);
			ПрилепляемыеЧастицы = ПрочитатьПрилепляемыеЧастицыВМетодеВВеткеСЗавязями(МетодЗавязи, ТипДуба, Определение);
			Завязь = СоздатьЗавязьЧерезМетодЗавязи(ТипДуба, МетодЗавязи);
			Прозвища = ПрочитатьПрозвища(Аннотации, ИмяЖелудяЗавязи);
			Порядок = ПрочитатьПорядок(Аннотации);
			Верховный = ПрочитатьПризнакВерховногоЖелудя(Аннотации);
...
Функция ПрочитатьТипЖелудяПИ(Имя, Аннотации)
...
Функция ПрочитатьПрилепляемыеЧастицыВМетодеВВеткеСЗавязями(Метод, ТипВладельцаСвойств, Определение)
	...
	Для Каждого ПараметрМетода Из Метод.Параметры Цикл
		...
		//зарезервированное имя параметра куда передается основание для желудя.
		Если ПараметрМетода.Имя = "Основание" Тогда
			Свойство = СформироватьФиктивноеСвойство(Определение);

		Иначе
			Свойство = ПараметрМетода;
		КонецЕсли;	

		ПрилепляемаяЧастица = ПрилеплениеЧастиц.ДанныеОПрилепляемойЧастице(Свойство);
		ПрилепляемыеЧастицы.Добавить(ПрилепляемаяЧастица);
...
Функция СформироватьФиктивноеСвойство(Определение)
	// Это прям колхоз-заглушка, наверняка можно сделать хорошо и правильно.
	Свойство = новый структура;
	Свойство.Вставить("Имя", "Основание");
	
	ТаблицаПараметровАннотации = Новый ТаблицаЗначений;
	ТаблицаПараметровАннотации.колонки.Добавить("Имя");
	ТаблицаПараметровАннотации.колонки.Добавить("Значение");
	СтрокаПараметров = ТаблицаПараметровАннотации.добавить();
	СтрокаПараметров.Имя = "Значение";
	СтрокаПараметров.Значение = Определение.Имя();

	ТаблицаАннотации = Новый ТаблицаЗначений;
	ТаблицаАннотации.колонки.Добавить("Имя");
	ТаблицаАннотации.колонки.Добавить("Параметры");
	СтрокаАннотаций = ТаблицаАннотации.добавить();
	СтрокаАннотаций.Имя = "Пластилин";
	СтрокаАннотаций.Параметры = ТаблицаПараметровАннотации;

	Свойство.Вставить("Аннотации", ТаблицаАннотации);
