...
Функция ПолучитьСписокОпределенийЖелудей(Имя) Экспорт
	...
	// Таблица сортировки живёт в отдельной переменной: нативный компилятор не позволяет
	// присвоить переменной значение другого типа, а Результат выше уже был массивом.
	ТаблицаОпределений = Новый ТаблицаЗначений();
	ТаблицаОпределений.Колонки.Добавить("Порядок");
	ТаблицаОпределений.Колонки.Добавить("ОпределениеЖелудя");

	НайденныеОпределения = ОпределенияЖелудейПоПрозвищу.Получить(Имя);

	Если НайденныеОпределения <> Неопределено Тогда
		Для Каждого Определение Из НайденныеОпределения Цикл
			Строка = ТаблицаОпределений.Добавить();
			Строка.ОпределениеЖелудя = Определение;
			Строка.Порядок = Определение.Порядок();
		КонецЦикла;
	КонецЕсли;

	ТаблицаОпределений.Сортировать("Порядок Возр");
	КопияРезультат = ТаблицаОпределений.ВыгрузитьКолонку("ОпределениеЖелудя");
