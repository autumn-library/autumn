...
Функция ЗарегистрироватьДуб(ТипДуба) Экспорт
	...
	Для Каждого МетодЗавязи Из МетодыЗавязи Цикл
		...
		Прозвище = ПрочитатьПрозвище(МетодЗавязи, ИмяЖелудяЗавязи);
...
Функция СоздатьОпределениеЖелудя(ТипЖелудя, Конструктор, АннотацияНадКонструктором, Знач ИмяЖелудя = "")
	...
	Прозвище = ПрочитатьПрозвище(Конструктор, ИмяЖелудя);
...
Функция ПрочитатьПрозвище(Метод, ЗначениеПоУмолчанию)
	Аннотация = РаботаСАннотациями.ПолучитьАннотацию(Метод, "Прозвище");
	Если Аннотация = Неопределено Тогда
		Возврат ЗначениеПоУмолчанию;
	КонецЕсли;

	Прозвище = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(
		Аннотация,
		,
		ЗначениеПоУмолчанию
	);
...
Процедура СохранитьОпределениеЖелудя(ОпределениеЖелудя)
	...
	СуществующиеИмена = ОпределенияЖелудейПоПрозвищу.Получить(ОпределениеЖелудя.Прозвище());
	Если СуществующиеИмена = Неопределено Тогда
		СуществующиеИмена = Новый Массив;
	КонецЕсли;
	СуществующиеИмена.Добавить(ОпределениеЖелудя);
	ОпределенияЖелудейПоПрозвищу.Вставить(ОпределениеЖелудя.Прозвище(), СуществующиеИмена);
