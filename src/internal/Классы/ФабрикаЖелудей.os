...
Функция ДобавитьДуб(ТипДуба) Экспорт
	...
	Для Каждого МетодЗавязи Из МетодыЗавязи Цикл
		...
		ТипЖелудя = ПрочитатьТипЖелудя(МетодЗавязи, МетодЗавязи.Аннотации, ТипДуба);

		Если НЕ УсловияВыполнены(МетодЗавязи.Аннотации) Тогда
			Лог.Отладка("Завязь %1 не зарегистрирована: условия не выполнены", ИмяЖелудя);
			Продолжить;
		КонецЕсли;
...
Функция ДобавитьЗавязьИзЛямбды(ЛямбдаЗавязи, Контекст = Неопределено) Экспорт
	...
	ТипЖелудя = ПрочитатьТипЖелудя(Завязь, Завязь.Аннотации, Объект);
	...
	Если НЕ УсловияВыполнены(Завязь.Аннотации) Тогда
		Лог.Отладка("Завязь из лямбды %1 не зарегистрирована: условия не выполнены", ИмяЖелудя);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ДобавитьНапильник(ТипНапильника) Экспорт
	...
	Если ОпределениеНапильника = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ДобавитьЖителяЛеса(ТипЖителяЛеса, ИмяЖителяЛеса, АннотацияНадКонструктором) Экспорт
	...
	Если НЕ УсловияВыполнены(Аннотации) Тогда
		Лог.Отладка("Желудь %1 не зарегистрирован: условия не выполнены", ИмяЖителяЛеса);
		Возврат Неопределено;
	КонецЕсли;
...
Функция УсловияВыполнены(Аннотации)

	АннотацииУсловий = РаботаСАннотациями.НайтиАннотации(Аннотации, "Условие");

	Если АннотацииУсловий.Количество() = 0 Тогда
		Возврат Истина;
	КонецЕсли;

	Для Каждого АннотацияУсловия Из АннотацииУсловий Цикл

		ИмяАннотацииУсловия = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(АннотацияУсловия);

		АннотацияПроверяемогоУсловия = РаботаСАннотациями.НайтиАннотацию(Аннотации, ИмяАннотацииУсловия);

		Если АннотацияПроверяемогоУсловия = Неопределено Тогда
			Продолжить;
		КонецЕсли;

		ОпределениеАннотацииУсловия = Поделка.ПолучитьОпределениеАннотации(ИмяАннотацииУсловия);
		ОбъектУсловия = ОпределениеАннотацииУсловия.СоздатьОбъектАннотации(АннотацияПроверяемогоУсловия);

		Если НЕ ОбъектУсловия.Совпадает(Поделка) Тогда
			Лог.Отладка("Условие %1 не выполнено", ИмяАннотацииУсловия);
			Возврат Ложь;
		КонецЕсли;
