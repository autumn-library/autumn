...
Функция НайтиЖелудь(ИмяЖелудя, ПрилепляемыеЧастицы) Экспорт
	...
	Иначе
		...
		Для Каждого ДанныеОПрилепляемойЧастице Из ОпределениеЖелудя.ПрилепляемыеЧастицы() Цикл
			...
			Иначе
				ПрилепляемаяЧастица = ПрилеплениеЧастиц.НайтиПрилепляемуюЧастицу(Поделка, УправляющийПрилепляемымиКоллекциями, ДанныеОПрилепляемойЧастице);
			КонецЕсли;
...
Процедура ПриСозданииОбъекта(пПоделка, пРазворачивательАннотаций, пУправляющийПрилепляемымиКоллекциями)
