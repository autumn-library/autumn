...
// ПодписчикНаПриемки - Стековый помощник для работы с обработчиками событий,
// см. комментарий в самом классе.
Перем ПодписчикНаПриемки;
...
Процедура ПросканироватьИзвестныеТипы() Экспорт
	...
	Для Каждого Приемка Из Приемки Цикл
		ПодписчикНаПриемки.Отписать(ФабрикаЖелудей, Приемка);
	КонецЦикла;
...
Процедура ДобавитьПриемку(Тип)
	...
	ПодписчикНаПриемки.Подписать(ФабрикаЖелудей, Приемка);
...
Процедура ПриСозданииОбъекта(пПоделка, пФабрикаЖелудей, пКонтейнерАннотаций, пУправляющийПрилепляемымиКоллекциями)
	...
	ПодписчикНаПриемки = Новый ПодписчикНаПриемки();
