...
Процедура ЗапуститьПриложение(ПараметрыЗапуска = Неопределено) Экспорт
	...
	Для Каждого ИмяРогатки Из Рогатки Цикл
		...
		ИначеЕсли РефлекторОбъекта.ЕстьПроцедура("ПриЗапускеПриложения", 1) Тогда
			Рогатка.ПриЗапускеПриложения(ПараметрыЗапуска);
		КонецЕсли;
	КонецЦикла;
