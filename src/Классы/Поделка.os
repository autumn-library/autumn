...
Функция ПросканироватьКаталог(Каталог) Экспорт
	...
	МассивТиповЖелудейДляПостОбработки = Новый Массив;
	...
	Для Каждого Файл Из Файлы Цикл
		...
		// Проверим, не нужна ли пост-инициализация файлу
		МассивИменАннотацийДляПостОбработки = НайтиИменаАннотацииДляВыполненияПоСостояниюПриложения(СостоянияПриложения.ПостИнициализация());
		Для каждого ИмяАннотации Из МассивИменАннотацийДляПостОбработки Цикл
			Если РаботаСАннотациями.НайтиМетодыСАннотацией(Методы, ИмяАннотации).Количество() > 0 Тогда
				МассивТиповЖелудейДляПостОбработки.Добавить(ТипЖелудя);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;

	// Это концепция, потому состояние приложения будет меняться здесь, потом надо будет подумать над тем, оставлять поделку в стадии пост-инициализации или нет?
	// тут надо написать нормальный человеческий обработчик аннотаций относительно этапа их исполнениия. сдеревом выполнения относительно этапов, как мне видится. Где будет Этап выполнения-файл-аннотации, но это если сама концепция нормальная
	СостояниеПриложения = СостоянияПриложения.ПостИнициализация(); //А надо ли отдельное состояние приложения? Или фазы в аннотации достаточно?

	Для каждого ТипЖелудя Из МассивТиповЖелудейДляПостОбработки Цикл
		РефлекторОбъекта = Новый РефлекторОбъекта(ТипЖелудя);
		Методы = РефлекторОбъекта.ПолучитьТаблицуМетодов(, Ложь);

		РазворачивательАннотаций.РазвернутьАннотацииСвойств(Методы, ТипЖелудя);

		Если РаботаСАннотациями.НайтиМетодыСАннотацией(Методы, "Дуб").Количество() > 0 Тогда
			ФабрикаЖелудей.ПостИнициализацияДуба(ТипЖелудя);
		Иначе // BSLLS:EmptyCodeBlock-off
			// no-op
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗапуститьПриложение() Экспорт
	...
	СостояниеВыполнение(); // Кривая заплатка, так же надо слегка переделать
...
Процедура СостояниеВыполнение()

	Если СостояниеПриложения = СостоянияПриложения.Выполнение() Тогда
		ВызватьИсключение "Приложение находится в состоянии выполнения. Операция запрещена.";
	КонецЕсли;
...
Функция НайтиИменаАннотацииДляВыполненияПоСостояниюПриложения(СостояниеПриложения)
	МассивИменАннотаций = Новый Массив();
	Для каждого ОпределениеАннотации Из КонтейнерАннотаций.ПолучитьОпределенияАннотаций() Цикл
		Если ОпределениеАннотации.Значение.МоментВыполнения() = СостояниеПриложения Тогда
			МассивИменАннотаций.Добавить(ОпределениеАннотации.Значение.Имя());
		КонецЕсли;
	КонецЦикла;

	Возврат МассивИменАннотаций;
КонецФункции
...
Процедура ПриСозданииОбъекта(Знач СоветДругогоМастера = Неопределено)
	...
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияПорядок"));
	...
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияТабакерка"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияФинальныйШтрих"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияЖелудь"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияДуб"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияЗавязь"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияВеткаСЗавязями"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияНапильник"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияЗаготовка"));
	КонтейнерАннотаций.ДобавитьАннотацию(Тип("АннотацияРогатка"));
	...
	ФабрикаЖелудей.ДобавитьДуб(Тип("СлужебныйДубОсени"));
