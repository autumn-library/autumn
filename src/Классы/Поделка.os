...
Функция ИнициализироватьЖелудьОдиночку(Имя, ПрилепляемыеЧастицы)
	...
	Если Желудь = Неопределено Тогда
		Попытка
			...
			Семафор.Освободить();
		Исключение
			ТекстСообщения = СтрШаблон("Не удалось инициализировать желудь %1", Имя);
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Лог.Ошибка(ТекстСообщения);
			Лог.Ошибка(ТекстОшибки);
			Семафор.Освободить();
			ВызватьИсключение СтрШаблон("%1; %2", ТекстСообщения, ТекстОшибки);
		КонецПопытки;
	КонецЕсли;
...
Процедура ЗахватитьСемафор(Семафор)
	Попытка
		Семафор.Захватить(50);
	Исключение
		ВызватьИсключение "Циклическое внедрение пластилина";
	КонецПопытки;
КонецПроцедуры
