...
Функция ЗарегистрироватьЖителяЛеса(ТипЖителяТеля, ИмяЖителяЛеса, АннотацияНадКонструктором)
	...
	Методы = РефлекторОбъекта.ПолучитьТаблицуМетодов(АннотацияНадКонструктором);
	Ожидаем.Что(
		Методы.Количество(), 
...
Функция ПрочитатьМаркиПластилинаВМетоде(Метод)
	...
	Для Каждого ПараметрМетода Из Метод.Параметры Цикл
		...
		АннотацииПараметра = ПараметрМетода.Аннотации;
		...
		// todo: умный анализ наличия значения аннотации
		...
		// МаркиПластилина = ПараметрыАннотации[0].Значение;
		
		МаркаПластилина = ПараметрМетода.Имя;
