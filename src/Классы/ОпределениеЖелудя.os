...
Процедура ПриСозданииОбъекта(
	...
	Если Спецификация = СостоянияПриложения.Инициализация() Тогда
		ТекстСообщения = СтрШаблон(
			"К желудю времени инициализации %1 можно прилеплять только детальки или другие желуди времени инициализации.",
			Имя
		);
		// Список известных желудей времени инициализации, доступных как служебные желуди
		ИзвестныеЖелудиИнициализации = Новый Массив;
		ИзвестныеЖелудиИнициализации.Добавить("КонтейнерАннотаций");
		ИзвестныеЖелудиИнициализации.Добавить("РазворачивательАннотаций");
		ИзвестныеЖелудиИнициализации.Добавить("ПрилепляторЧастиц");
		ИзвестныеЖелудиИнициализации.Добавить("МенеджерНапильников");
		ИзвестныеЖелудиИнициализации.Добавить("РепозиторийОпределенийЖелудей");
		ИзвестныеЖелудиИнициализации.Добавить("ЧитательАннотаций");
		ИзвестныеЖелудиИнициализации.Добавить("ФабрикаЗавязей");
		
		Для Каждого ПрилепляемаяЧастица Из ПрилепляемыеЧастицы Цикл
			Разрешено = (ПрилепляемаяЧастица.ТипЧастицы() = ТипыПрилепляемыхЧастиц.Деталька()) 
				ИЛИ (ПрилепляемаяЧастица.ИмяЧастицы() = "Поделка")
				ИЛИ (ИзвестныеЖелудиИнициализации.Найти(ПрилепляемаяЧастица.ИмяЧастицы()) <> Неопределено);
