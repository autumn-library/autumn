...
Настройки запуска бенчмарков (итерации, прогрев, количество вызовов) задаются программно через `КонфигурацияБенчмарков` в наборе тестов, а не аннотациями benchos. Причина: при создании поделки осень сканирует все известные типы, регистрирует аннотации benchos как определения аннотаций и строго проверяет их параметры, а параметризованные аннотации benchos (`&КоличествоИтераций`, `&ПередКаждымКейсом` и другие) объявлены без параметров - применение их в классе бенчмарка, поднимающего поделку, падает с ошибкой валидации
