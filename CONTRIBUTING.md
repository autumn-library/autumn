...
Тесты производительности живут отдельно и запускаются через [benchos](https://github.com/Stivo182/BenchmarkOneScript):

```cmd
oscript tasks/performance.os
```
...
3. Открывай файл, который ты хочешь запустить с отладкой(например [performance.os](tasks/performance.os))

4. Нажимай кнопку "Начать отладку"

Отдельные тесты так запустить не получится - это не самостоятельные сценарии. Для них подними сервер отладки самим oneunit и подключись к нему конфигурацией "Подключение к oneunit":
...
* [Бенчмарки](performance_tests) здесь собираются svt-тесты. Это не тесты, а бенчмарки [benchos](https://github.com/Stivo182/BenchmarkOneScript): методы с аннотацией `&Бенчмарк`, по одной замеряемой операции в каждом. Пороги времени выполнения задаются в [tasks/performance.os](tasks/performance.os), туда же надо добавить порог для нового бенчмарка - без порога прогон падает
