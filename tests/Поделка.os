...
&Тест
Процедура ПроверкаИсключенияНаЦикличномВнедрении() Экспорт
	// Дано 
	Поделка = Новый Поделка();
	Поделка.ДобавитьЖелудь(Тип("ПервыйЖелудьСЦикличныйЗависимостью"));
	Поделка.ДобавитьЖелудь(Тип("ВторойЖелудьСЦикличныйЗависимостью"));
	Поделка.ЗапуститьПриложение();

	// Когда
	Параметры = Новый Массив();
	Параметры.Добавить("ПервыйЖелудьСЦикличныйЗависимостью");

	// Тогда
	Ожидаем.Что(Поделка).Метод("НайтиЖелудь", Параметры).ВыбрасываетИсключение("Циклическое внедрение пластилина");
