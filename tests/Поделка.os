...
&Тест
&Изолированный // Ввиду мутации персистентной структуры
...
&Тест
Процедура ЗавязьВернувшаяНеопределеноНеПопадаетВМассивПоПрозвищу() Экспорт

	// Дано-Когда
	Желуди = Поделка.НайтиЖелуди("ВозможноПустой");

	// Тогда
	Ожидаем
		.Что(Желуди, "В массиве по прозвищу должен остаться только заполненный желудь")
		.ИмеетДлину(1);

	Ожидаем
		.Что(Желуди[0], "Единственный желудь в массиве должен быть заполненным")
		.Не_().Равно(Неопределено);

	Ожидаем
		.Что(Желуди[0].Имя)
		.Равно("ЗаполненнаяВозможноПустаяЗавязь");

КонецПроцедуры

&Тест
Процедура ЗавязьВернувшаяНеопределеноНеПопадаетВСоответствиеПоПрозвищу() Экспорт

	// Дано-Когда
	Желуди = Поделка.НайтиЖелуди("ВозможноПустой",, "Соответствие");

	// Тогда
	Ожидаем
		.Что(Желуди, "В соответствии по прозвищу должен остаться только заполненный желудь")
		.ИмеетДлину(1);

	Ожидаем
		.Что(Желуди.Получить("ЗаполненнаяВозможноПустаяЗавязь"), "Заполненный желудь должен быть в соответствии")
		.Не_().Равно(Неопределено);

	Ожидаем
		.Что(Желуди.Получить("ПустаяЗавязь"), "Ключа пустой завязи в соответствии быть не должно")
		.Равно(Неопределено);

КонецПроцедуры

&Тест
Процедура ЗавязьВернувшаяНеопределеноНеВнедряетсяВКоллекцииЧерезПластилин() Экспорт

	// Дано-Когда
	Желудь = Поделка.НайтиЖелудь("ЖелудьСВозможноПустымиЗавязями");

	// Тогда
	Ожидаем
		.Что(Желудь.ВнедряемыеЖелуди, "Во внедрённом массиве должен остаться только заполненный желудь")
		.ИмеетДлину(1);

	Ожидаем
		.Что(Желудь.ВнедряемыеЖелуди[0], "Единственный внедрённый желудь должен быть заполненным")
		.Не_().Равно(Неопределено);

	Ожидаем
		.Что(Желудь.ВнедряемыеЖелудиПоИменам, "Во внедрённом соответствии должен остаться только заполненный желудь")
		.ИмеетДлину(1);
...
&Изолированный(Совет = "() -> {
	...
	|			Соответствия.ИзЭлементов(""Моя"", Новый Соответствие(
	|				Соответствия.ИзЭлементов(""Настройка"", ""Переопределенное значение"")
	|			)))
	|		);
	|}"
)
...
Процедура ЖелудьПолучаетБлестяшкуВКонструктор() Экспорт
	...
	// Когда-Тогда
	Желудь = Поделка.НайтиЖелудь("ЖелудьСБлестяшкой", Массивы.ИзЭлементов(ЗначениеБлестяшки));

	// Тогда
	Ожидаем
		.Что(Желудь.ПолучитьХранимуюБлестяшку())
		.Равно(ЗначениеБлестяшки)
	;
...
Процедура ЖелудьПолучаетБлестяшкуПластелинДеталькуВКонструктор() Экспорт
	...
	// Когда-Тогда
	Желудь = Поделка.НайтиЖелудь(
		"ЖелудьСБлестяшкойПластелиномДеталькой",
		Массивы.ИзЭлементов(ЗначениеБлестяшки, ЗначениеВторойБлестяшки)
	);
	...
	;
	Ожидаем
		.Что(Желудь.ПолучитьВторуюХранимуюБлестяшку())
		.Равно(ЗначениеВторойБлестяшки)
	;
КонецПроцедуры
...
Процедура ДобавитьЗавязьБезТипа() Экспорт
	...
	Ожидаем.Что(Поделка)
		...
		.ВыбрасываетИсключение("Тип желудя в лямбда-завязи не известен. Укажите тип желудя в аннотации &Завязь. Лямбда: &Завязь ()-> Возврат 1;");
...
Процедура ДобавитьЗавязьСКонтекстом() Экспорт
	...
	Ожидаем.Что(Поделка.НайтиЖелудь("ДобавитьКЧислу", Списки.ИзЭлементов(21).ВМассив()))
		.Равно(42);
...
// Поле МенеджерПараметров поделки хранит МенеджерПараметров configor, а не строитель.
// Строитель - объект настройки состава провайдеров, менеджер - объект времени выполнения,
// и внешние библиотеки (например autumn-opentelemetry) достают из поделки именно менеджер,
// чтобы передать его своей автоконфигурации с полным интерфейсом configor.
&Тест
Процедура ПоделкаХранитМенеджерПараметровConfigor() Экспорт

	// Дано
	НоваяПоделка = Новый Поделка();

	// Когда
	Менеджер = (Новый Рефлектор()).ПолучитьСвойство(НоваяПоделка, "МенеджерПараметров");

	// Тогда
	Ожидаем.Что(ТипЗнч(Менеджер)).Равно(Тип("МенеджерПараметров"));
	Ожидаем.Что(Менеджер.ЧтениеВыполнено(), "чтение параметров выполнено").Равно(Истина);
