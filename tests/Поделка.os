...
&Изолированный(
	Уровень = "Процесс",
	Импорты = "./ВложенныеЗначения",
	Совет = "() -> {
		|	Возврат Новый СоветДругогоМастера()
		|		.ЗначенияДеталек(Новый Соответствие(
		|			Соответствия.ИзЭлементов(""ИмяАппендера"", ""ЦелевойЖелудь"")
		|		));
		|}"
)
Процедура ИмяЖелудяРезолвитсяИзВложеннойДетальки() Экспорт

	// Дано-Когда
	// &Пластилин(&Деталька("ИмяАппендера")) имя внедряемого желудя резолвится из настройки (значение вложенной детальки).
	Желудь = Поделка.НайтиЖелудь("ЖелудьСВложеннойДеталькой");

	// Тогда
	Ожидаем.Что(Желудь.Зависимость, "Вложенная деталька не разрешилась в имя желудя").ИмеетТип("ЦелевойЖелудь");
