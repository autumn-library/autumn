...
СПомощьюПроизвольнойАннотацииМожноПолучитьЖелудьПоПрозвищу();

// ТаблицаМетодов = Новый Рефлектор().ПолучитьТаблицуМетодов(Тип("АннотацияАннотация"));
// Ожидаем.Что(ТаблицаМетодов[0].Параметры[0].ПоЗначению).Равно(Ложь);
// Ожидаем.Что(ТаблицаМетодов[0].Параметры[0].ЕстьЗначениеПоУмолчанию).Равно(Ложь);

// Ожидаем.Что(ТаблицаМетодов[0].Параметры[1].ПоЗначению).Равно(Ложь);
// Ожидаем.Что(ТаблицаМетодов[0].Параметры[1].ЕстьЗначениеПоУмолчанию).Равно(Истина);