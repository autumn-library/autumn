...
// TODO: Отключаю до лучших времен. Порядок не реализован.
// &Тест
...
Процедура ВыкидываетИсключениеНаНеизвестноеИмяЖелудя() Экспорт
	...
	// Тогда
	Ожидаем.Что(Поделка, "Поиск несуществующего желудя вызывает исключение").Метод("НайтиЖелудь", ПараметрыМетода).ВыбрасываетИсключение("Не удалось получить определение Желудя");
