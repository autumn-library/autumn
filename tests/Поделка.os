...
#Область ТестыУсловнойРегистрации

&Тест
Процедура ЖелудьРегистрируетсяПриВыполненномУсловииНаСвойство() Экспорт

	// Дано
	УстановитьТекущийКаталог(ОбъединитьПути(ТекущийКаталог(), "tests/КаталогНастроекСУсловиями"));
	Поделка = Новый Поделка();
	Поделка.ЗапуститьПриложение();

	// Когда
	ОпределениеЖелудя = Поделка.ПолучитьОпределениеЖелудя("ЖелудьСУсловиемИстина");

	// Тогда
	Ожидаем.Что(ОпределениеЖелудя, "Желудь с выполненным условием должен быть зарегистрирован").Не_().Равно(Неопределено);

КонецПроцедуры

&Тест
Процедура ЖелудьНеРегистрируетсяПриНевыполненномУсловииНаСвойство() Экспорт
	...
	// Когда
	ОпределениеЖелудя = Поделка.ПолучитьОпределениеЖелудя("ЖелудьСУсловиемЛожь");

	// Тогда
	Ожидаем.Что(ОпределениеЖелудя, "Желудь с невыполненным условием не должен быть зарегистрирован").Равно(Неопределено);

КонецПроцедуры

&Тест
Процедура ЖелудьНеРегистрируетсяПриОтсутствииСвойства() Экспорт
	...
	// Когда
	ОпределениеЖелудя = Поделка.ПолучитьОпределениеЖелудя("ЖелудьСУсловиемОтсутствующее");

	// Тогда
	Ожидаем.Что(ОпределениеЖелудя, "Желудь с условием на несуществующее свойство не должен быть зарегистрирован").Равно(Неопределено);

КонецПроцедуры

&Тест
Процедура ЖелудьРегистрируетсяПриОтсутствииСвойстваСФлагомСопоставления() Экспорт
	...
	// Когда
	ОпределениеЖелудя = Поделка.ПолучитьОпределениеЖелудя("ЖелудьСУсловиемОтсутствующееПоУмолчанию");

	// Тогда
	Ожидаем.Что(ОпределениеЖелудя, "Желудь с СопоставлятьЕслиОтсутствует = Истина должен быть зарегистрирован").Не_().Равно(Неопределено);

КонецПроцедуры

&Тест
Процедура ЖелудьРегистрируетсяПриКастомномОжидаемомЗначении() Экспорт
	...
	// Когда
	ОпределениеЖелудя = Поделка.ПолучитьОпределениеЖелудя("ЖелудьСКастомнымУсловием");

	// Тогда
	Ожидаем.Что(ОпределениеЖелудя, "Желудь с кастомным ожидаемым значением должен быть зарегистрирован").Не_().Равно(Неопределено);

КонецПроцедуры

&Тест
Процедура УсловиеНаЗавязиВДубе() Экспорт
	...
	// Когда
	ОпределениеСУсловием = Поделка.ПолучитьОпределениеЖелудя("ЖелудьИзЗавязиСУсловиемИстина");
	ОпределениеБезУсловия = Поделка.ПолучитьОпределениеЖелудя("ЖелудьИзЗавязиСУсловиемЛожь");

	// Тогда
	Ожидаем.Что(ОпределениеСУсловием, "Завязь с выполненным условием должна быть зарегистрирована").Не_().Равно(Неопределено);
	Ожидаем.Что(ОпределениеБезУсловия, "Завязь с невыполненным условием не должна быть зарегистрирована").Равно(Неопределено);
