...
&Тест
Процедура ДубСоздаетЖелудиСВетки() Экспорт

	// Дано
	Поделка = Новый Поделка();
	Каталог = ОбъединитьПути(ТекущийКаталог(), "tests", "ЖелудиСущности", "Классы");
	...
	// Когда
	ЗавязьНаВетке1 = Поделка.НайтиЖелудь("Менеджер_ЖелудьСущность1");

	// Тогда
	Ожидаем.Что(ЗавязьНаВетке1).Не_().Равно(Неопределено);
	Ожидаем.Что(ЗавязьНаВетке1.Ключ).Равно(1);

		// Когда
	ЗавязьНаВетке2 = Поделка.НайтиЖелудь("Менеджер_ЖелудьСущность2");

	// Тогда
	Ожидаем.Что(ЗавязьНаВетке2).Не_().Равно(Неопределено);
	Ожидаем.Что(ЗавязьНаВетке2.Ключ).Равно(2);
