...
Процедура ДефолтныйСоставПровайдеровСоздан() Экспорт
	...
	Ожидаем.Что(Строитель.Провайдер("inline")).Не_().Равно(Неопределено);
	Ожидаем.Что(Строитель.Провайдер("inline").Приоритет).Равно(3);
...
Процедура МенеджерПараметровДоступенИЕдинственный() Экспорт
	...
	// Когда
	Менеджер = Строитель.МенеджерПараметров();

	// Тогда
	Ожидаем.Что(Менеджер).Не_().Равно(Неопределено);
	Ожидаем.Что(Строитель.МенеджерПараметров()).Равно(Менеджер);
...
Процедура ДобавитьПровайдерСоответствиеРегистрируетInlineПровайдер() Экспорт
	...
	Менеджер = Строитель.МенеджерПараметров();
	Менеджер.Прочитать();
	Ожидаем.Что(Менеджер.Параметр("ключ")).Равно("значение");
...
Процедура УдалитьПровайдерОтключаетПровайдер() Экспорт
	...
	// Тогда
	Ожидаем.Что(Строитель.Провайдер("inline").Включен).Равно(Ложь);
...
Процедура ОчиститьПровайдерыСбрасываетВсеПровайдеры() Экспорт
	...
	Ожидаем.Что(Строитель.Провайдер("inline")).Равно(Неопределено);
