...
В каталоге `performance_tests` лежат бенчмарки библиотеки benchos, документация - https://github.com/Stivo182/BenchmarkOneScript. Запускаются командой:

```sh
oscript tasks/performance.os
```

Сценарий прогоняет все бенчмарки каталога и сравнивает среднее время выполнения операции с порогами, заданными в самом сценарии. Для каждого бенчмарка порог обязателен.
