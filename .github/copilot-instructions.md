...
Тесты выполняются движком oneunit, документация - https://github.com/sfaqer/oneunit. Он ставится вместе с остальными зависимостями разработки, исполняемый файл лежит в `oscript_modules/bin`.
...
Без опций прогоняются все наборы из каталога `tests`. Отдельный набор запускается опцией `-f`, отдельные тесты - опцией `-t`.

Тесты, которые портят общий прогон, помечаются в наборе аннотацией `&Изолированный(Уровень = "Процесс", Импорты = "...")` и выполняются в отдельном процессе.
...
Настройки запуска бенчмарков задаются программно через `КонфигурацияБенчмарков` в наборе тестов. Аннотации benchos с параметрами в классах бенчмарков использовать нельзя: при создании поделки осень сканирует все известные типы, регистрирует аннотации benchos как определения аннотаций и строго проверяет их параметры, а параметризованные аннотации benchos объявлены без параметров.
